...
def create_example_lcao_output():
    """
    Create example LCAO output matching EXACT parser logic.
    
    Parser flow:
    - Reads spin channel label once → stores in current_spin_channel
    - All subsequent Fock matrices use that spin channel
    - Until a new spin channel label is encountered
    
    Structure:
    1. Lattice vectors
    2. All overlap matrices (no spin labels)
    3. ALPHA_ALPHA label
    4. All ALPHA_ALPHA Fock matrices (real+imag for all R vectors)
    5. BETA_BETA label
    6. All BETA_BETA Fock matrices (real+imag for all R vectors)
    7. ALPHA_BETA label
    8. All ALPHA_BETA Fock matrices (real+imag for all R vectors)
    9. BETA_ALPHA label
    10. All BETA_ALPHA Fock matrices (real+imag for all R vectors)
