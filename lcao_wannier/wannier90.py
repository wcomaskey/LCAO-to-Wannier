...
def write_dmn_file(
    filename: str,
    ksym_map,
    d_matrix_wann: np.ndarray,
    d_matrix_band: np.ndarray,
    num_bands: int,
    num_wann: int,
    num_kpts: int,
    verbose: bool = True
    ...
    """
    Write a .dmn file for Wannier90 site_symmetry mode.

    File format matches pw2wannier90.f90 output and is read by sitesym.F90.
    All indices in the file are 1-based (Fortran convention).

    Parameters
    ----------
    filename : str
        Output filename (e.g., "seedname.dmn").
    ksym_map : KPointSymmetryMap
        K-point symmetry mapping (0-based internally).
    d_matrix_wann : ndarray (num_wann, num_wann, nsymmetry, nkptirr)
        Wannier-space representation matrices.
    d_matrix_band : ndarray (num_bands, num_bands, nsymmetry, nkptirr)
        Band-space representation matrices.
    num_bands : int
        Number of bands.
    num_wann : int
        Number of Wannier functions.
    ...
    with open(filename, 'w') as f:
        ...
        f.write(f"  Generated by LCAO-to-Wannier90 on {timestamp}\n")

        # Line 2: num_bands, nsymmetry, nkptirr, num_kpts
        f.write(f"{num_bands:9d}{nsymmetry:9d}{nkptirr:9d}{num_kpts:9d}\n")

        # ik2ir (1-based)
        f.write("\n")
        ik2ir_1based = ksym_map.ik2ir + 1
        _write_int_array(f, ik2ir_1based)

        # ir2ik (1-based)
        f.write("\n")
        ir2ik_1based = ksym_map.ir2ik + 1
        _write_int_array(f, ir2ik_1based)

        # kptsym: for each ir, write kptsym(:, ir) (1-based)
        for ir in range(nkptirr):
            f.write("\n")
            kptsym_1based = ksym_map.kptsym[:, ir] + 1
            _write_int_array(f, kptsym_1based)

        # d_matrix_wann: for each ir, for each isym, write (num_wann x num_wann) matrix
        for ir in range(nkptirr):
            for isym in range(nsymmetry):
                f.write("\n")
                _write_complex_matrix(f, d_matrix_wann[:, :, isym, ir])

        # d_matrix_band: for each ir, for each isym, write (num_bands x num_bands) matrix
        for ir in range(nkptirr):
            for isym in range(nsymmetry):
                f.write("\n")
                _write_complex_matrix(f, d_matrix_band[:, :, isym, ir])

    if verbose:
        print(f"  ✓ {filename}: nsym={nsymmetry}, nkptirr={nkptirr}/{num_kpts}, "
              f"num_wann={num_wann}, num_bands={num_bands}")


def _write_int_array(f, arr: np.ndarray) -> None:
    """Write integer array, 10 per line, format i9."""
    for i, val in enumerate(arr):
        f.write(f"{int(val):9d}")
        if (i + 1) % 10 == 0 or i == len(arr) - 1:
            f.write("\n")


def _write_complex_matrix(f, mat: np.ndarray) -> None:
    """Write complex matrix in Fortran column-major order, one element per line."""
    nrow, ncol = mat.shape
    for j in range(ncol):       # column-major: j (column) is outer
        for i in range(nrow):   # i (row) is inner/fastest
            z = mat[i, j]
            f.write(f" ({z.real:18.10e},{z.imag:18.10e})\n")