...
def get_rot_trans(myposwan):
    ...
    cell_tuple = (crystal_ase.cell, crystal_ase.get_scaled_positions(), crystal_ase.get_atomic_numbers())

    symmetry = spglib.get_symmetry(cell_tuple)
    ...
    print(("Spacegroup of this cell is %s." % spglib.get_spacegroup(cell_tuple)))
    ...
    for isym in range(nsymm):
        print(("  --------------- %4d ---------------" % (isym + 1)), file=f)
        rot = symmetry['rotations'][isym]
        trans = symmetry['translations'][isym]
        print(("  rotation:"), file=f)
        for x in rot:
            print(("      %2d %2d %2d " % (x[0], x[1], x[2])), file=f)
        print(("  translation:"), file=f)
        print(("      %8.5f %8.5f %8.5f " % (trans[0], trans[1], trans[2])), file=f)
        print(("  rotmap:"), file=f)
        gtrans = trans
...
# rot map for wann.in. here
    ...
    print("rotations as for atoms in wannier projection", file=f)
    # rot map of atoms in wannier projection
    for isymm in range(nsymm):
        print("isymm", isymm+1, "---------", file=f)
        for iptr in range(nptrans):
            print("trans", 0, gtrans, file=f)
            for ii in range(natom_wan):
                print("OUTCAR atom rot map:", atom_num[ii], "->", symop[isymm][2][atom_num[ii]-1][1], "=====>", "wannier atom rot map:", ii+1, "->", wann_atom_rotmap[isymm,iptr,ii]+1, file=f)
    f.close()
