#!/bin/bash
# MgB2 (alpha) with the ORIGINAL PDWF window selection (NO --spread-window-assist).
set -u
...
SEED=MgB2
ARGS="--spin alpha --method pdwf --k-grid 8 8 8"   # no spread-window-assist
WORK=$HOME/mgb2_pdwf; rm -rf "$WORK"; mkdir -p "$WORK"; cd "$WORK" || exit 1

echo "######## Stage 1 $(date) ########"
python3 "$REPO/lcao_to_wannier90.py" --stage 1 --input "$IN" --seedname "$SEED" $ARGS --bands-plot 2>&1 \
    | grep -iE 'dis_froz|dis_win|num_wann:|num_bands|STATUS|Created:'
...
python3 "$REPO/scripts/plot_band_comparison.py" --input "$IN" --seedname "$SEED" \
    --spin alpha -o "${SEED}_pdwf.png"
ls -la "${SEED}_pdwf.png" 2>&1
echo "=== DONE $(date) ==="
